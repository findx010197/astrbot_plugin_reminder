...
- `/callme <时间> <事件>` - 快速创建提醒
- `/提醒我 <时间> <事件>` - 中文别名
- `/callme every <周期> <时间> <事件>` - 创建循环提醒
- `/callme list [all|today|recurring]` - 查看提醒列表
  - 无参数：显示菜单
  - `all`: 查看所有日程
  - `today`: 查看今日日程
  - `recurring`: 查看循环日程
...
/callme every 每天 21:00 提醒睡觉
```

**每周循环：**
```
/callme every week 一 14:00 周报
/callme every 每周 五 18:00 周末安排
```
支持的星期表达：一、二、三、四、五、六、日（或1-7）

**每月循环：**
```
/callme every month 15 10:00 交房租
/callme every 每月 1 9:00 月度总结
```

**循环日程说明：**
- 循环提醒在触发后会自动重新调度到下一个周期
- 在提醒列表中会显示🔁标记
- 取消循环提醒与普通提醒相同，使用 `/callme cancel <ID>`
...
用户: /callme list recurring
Bot: 🔁 循环日程
     R0005 │ 明天 09:00 🔁每天
           └─ 晨会
     R0007 │ 02/05 14:00 🔁每周一
           └─ 周报
```
